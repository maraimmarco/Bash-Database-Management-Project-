...
# Display a Zenity dialog to select a database for deletion
...
if [[ ! -d $db_to_drop ]]; then
    zenity --error --text="Database does not exist: $db_to_drop"
    exit 1
fi
...
if [[ $confirmation == "TRUE" ]]; then
    rm -rf "$db_to_drop"
    zenity --info --text="Database dropped: $db_to_drop"
else
...
;;
    ...
    if [[ -z $conDB ]]; then
        ...
        if [[ -d $conDB ]]; then  # Check if the entered database name is a directory
			...
			rm -f "$table_to_drop" "$metadata_file"
